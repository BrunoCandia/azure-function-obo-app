...
app.http("retrieve-files", {
  ...
  handler: async (request, context) => {
    ...
    context.log(`initGraphClient executed`);
    ...
    const getOboToken = async (userAccessToken) => {
      const { TENANT_ID, CLIENT_ID, MICROSOFT_PROVIDER_AUTHENTICATION_SECRET } =
        process.env;
    ...
    context.log(`getOboToken executed`);
    ...
    const getDriveItemContent = async (client, driveId, itemId, name) => {
      try {
        ...
        throw new Error(
          `Failed to fetch content for ${name}: ${error.message}`
        );
      }
    };

    context.log(`getDriveItemContent executed`);
    ...
    module.exports = async function (context, req) {
      ...
      const searchTerm =
        req.query.searchTerm || (req.body && req.body.searchTerm);
          ...
          await Promise.all(
            list.value[0].hitsContainers.map(async (container) => {
              for (const hit of container.hits) {
                if (
                  hit.resource["@odata.type"] === "#microsoft.graph.driveItem"
                ) {
                  const { name, id } = hit.resource;
                  const driveId = hit.resource.parentReference.driveId;
                  const contents = await getDriveItemContent(
                    client,
                    driveId,
                    id,
                    name
